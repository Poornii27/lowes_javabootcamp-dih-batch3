...
public class AccountDaoJDBCImpl implements AccountDao {
    ...
    public boolean createAccount(Account account) throws SQLException {
        ...
        try {
            conn = getConnection(DB_URL,USER,PASS);
            String query = "INSERT INTO account (name,type,balance,active) values (?,?,?,?)";
    ...
    public List<Account> getAccounts() throws SQLException {
        ...
        try {
            ...
            while(rs.next())
            {
                ...
                accnts.add(new Account(id,name,type,balance,active));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean updateAccount(int id, Account account) throws SQLException {
        ...
        try {
            conn = getConnection(DB_URL,USER,PASS);
            String query = "UPDATE account SET name = ?, type = ?,balance = ?,active = ? WHERE id = ?";
            pstmt = conn.prepareStatement(query);
            pstmt.setString(1, account.getName());
            pstmt.setString(2, account.getType());
            pstmt.setDouble(3, account.getBalance());
